...
from SchoolFinal1.ui_FINAL import Ui_MainWindow
...
from SchoolFinal1.studentDialog_FINAL import Ui_StudentsDialog
...
class MySideBar(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.removeStudentBtn.clicked.connect(self.remove_Student)
    ...
    def remove_Student(self):
        ...
        if self.select_student:
            SQLServerDoWork.DeleteStudent(self.select_student)#n5leh id
            self.loadData()
        else:
            print("Please select a student to remove.")
    ...
    def loadData(self):
        ...
        headers = ["Name", "ID", "Gender", "Class", "DOB", "Age", "Address", "Phone", "Email", "Select"]
        self.studentInfo_table.setHorizontalHeaderLabels(headers)
        ...
        for row, student in enumerate(students):
            ...
            #self.studentInfo_table.setItem(row, 9, QTableWidgetItem(str(student[9])))
            radio_button = QRadioButton()
            self.studentInfo_table.setCellWidget(row, 9, radio_button)
            radio_button = QRadioButton()
            radio_button.clicked.connect(lambda _, id=student[0]: self.set_selected_student(id))
            self.studentInfo_table.setCellWidget(row, 9, radio_button)
    ...
    def loadDataViolence(self):
        ...
        for row, video in enumerate(violence_videos):
            ...
            radio_button = QRadioButton()
            self.violence_reoprts_table.setCellWidget(row, 1, radio_button)
            radio_button = QRadioButton()
    ...
    #remove
    def set_selected_student(self, id):
        self.select_student = id
    ...
    def LoginForm(self):
        # Instantiate and show the login dialog
        login_dialog = Ui_Dialog(self)

        # Connect the clicked signal of the "loginBtn" button to the OnClickLoginButton method
        login_dialog.loginBtn.clicked.connect(lambda: self.OnClickLoginButton(login_dialog))

        # Execute the dialog (this will block until the dialog is closed)
        result = login_dialog.exec()

        # Check the result of the dialog execution (QDialog.Accepted or QDialog.Rejected)
        if result == QDialog.Accepted:
            # Proceed to the main window
            self.main_window = MySideBar()
            self.main_window.show()
        else:
            # Handle the case where the dialog was rejected
            print("Login cancelled or failed.")

    def OnClickLoginButton(self, login_dialog):
        import SQLServerDoWork
        username = login_dialog.email_lineEdit.text()
        password = login_dialog.password_lineEdit.text()

        if not username or not password:
            # Display message indicating invalid input
            QMessageBox.warning(login_dialog, "Invalid Input", "Please fill in all fields.")
            return

        if SQLServerDoWork.login(username, password):
            # Login successful
            print("Login successful")
            login_dialog.accept()  # Close the login dialog
        else:
            # Login failed
            QMessageBox.warning(login_dialog, "Login Failed", "Incorrect username or password.")
